...
-- ==============================================================================
-- Fix screen API functions to work with VARCHAR(7) month_ref after migration 0006
-- ==============================================================================
-- Migration 0004 created functions with DATE parameters/returns, but migration 0006
-- converts month_ref columns to VARCHAR(7). This migration updates those functions.

-- Drop the functions that were created in 0004 with DATE types
...
DROP FUNCTION IF EXISTS inflacao_brasil.get_wage_equivalence_and_villains(DATE);
DROP FUNCTION IF EXISTS inflacao_brasil.get_historical_series(DATE, DATE);

-- Recreate with VARCHAR(7) parameters and return types
-- Screen 1: Summary dashboard (basket total + top 10 items with MoM)
CREATE OR REPLACE FUNCTION inflacao_brasil.get_dashboard_summary(p_month_ref VARCHAR(7) DEFAULT to_char(CURRENT_DATE, 'YYYY-MM'))
RETURNS TABLE (
    month_ref VARCHAR(7),
...
CREATE OR REPLACE FUNCTION inflacao_brasil.get_dashboard_items(p_month_ref VARCHAR(7) DEFAULT to_char(CURRENT_DATE, 'YYYY-MM'))
RETURNS TABLE (
    item_id BIGINT,
    item_name TEXT,
...
-- Screen 2: Wage equivalence + top villains
CREATE OR REPLACE FUNCTION inflacao_brasil.get_wage_equivalence_and_villains(p_month_ref VARCHAR(7) DEFAULT to_char(CURRENT_DATE, 'YYYY-MM'))
RETURNS TABLE (
    month_ref VARCHAR(7),
...
wage_current AS (
    ...
    WHERE mwh.effective_from = (
        ...
        WHERE effective_from <= (p_month_ref || '-01')::DATE
    )
),
basket_with_wage AS (
    SELECT
...
-- Screen 3: Historical series
CREATE OR REPLACE FUNCTION inflacao_brasil.get_historical_series(
    p_start_month VARCHAR(7) DEFAULT '2022-07',
    p_end_month VARCHAR(7) DEFAULT to_char(CURRENT_DATE, 'YYYY-MM')
)
RETURNS TABLE (
    month_ref VARCHAR(7),
    series_type TEXT,
...
ipca_series AS (
    SELECT
        ...
        WHERE ipca.month_ref >= GREATEST(p_start_month, '2026-01')
